...
export default {
  ...
  methods: {
    ...
    checkValidISBN13(data) {
      ...
      const joinnedISBN = data.join("");
      sum % 10 === 0
        ? this.printIt(joinnedISBN + " is a Valid ISBN number", "success")
        : this.printIt("Not a valid ISBN", "error");
    },
    clearMessages() {
      const checkDigitInputs = document.getElementById("check-digit-inputs");
      if(checkDigitInputs.childNodes.length>2){
        checkDigitInputs.removeChild(checkDigitInputs.childNodes[2])
      }
    },
  },
};
